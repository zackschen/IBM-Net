...
def conv7x7(in_planes: int, out_planes: int, stride: int=1) -> F.conv2d:
    """
    Instantiates a 3x3 convolutional layer with no bias.
    :param in_planes: number of input channels
    :param out_planes: number of output channels
    ...
    :return: convolutional layer
    """
    return nn.Conv2d(in_planes, out_planes, kernel_size=7, stride=2,
...
class ResNet(MammothBackbone):
    ...
    def __init__(self, block: BasicBlock, num_blocks: List[int],
                 num_classes: int, ntask: int, nf: int, args = None) -> None:
        """
    ...
    def forward(self, x: torch.Tensor, task_id: int = None, returnt='out') -> torch.Tensor:
        """
        Compute a forward pass.
        ...
        if task_id is not None:
            out = self.linear[task_id](feature)
        else:
...
def resnet18(nclasses: int, ntask: int, nf: int=64, args = None) -> ResNet:
    """
    ...
    """
    return ResNet(BasicBlock, [2, 2, 2, 2], nclasses, ntask, nf, args)
