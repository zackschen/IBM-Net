...
class ContinualDataset:
    ...
    @staticmethod
    @abstractmethod
    def get_backbone(args) -> nn.Module:
        """
...
def store_masked_loaders_ImageNet(train_dataset: datasets, test_dataset: datasets,
    ...
    """
    train_mask = np.where((np.array(train_dataset.targets) >= setting.i) &
        (np.array(train_dataset.targets) < setting.i + setting.N_CLASSES_PER_TASK))
    test_mask = np.where((np.array(test_dataset.targets) >= setting.i) &
        (np.array(test_dataset.targets) < setting.i + setting.N_CLASSES_PER_TASK))

    train_dataset.samples = [train_dataset.samples[i] for i in train_mask[0]]
    train_dataset.data = [train_dataset.data[i] for i in train_mask[0]]
    train_dataset.targets = np.array([train_dataset.targets[i] for i in train_mask[0]])

    test_dataset.samples = [test_dataset.samples[i] for i in test_mask[0]]
    test_dataset.data = [test_dataset.data[i] for i in test_mask[0]]
    test_dataset.targets = np.array([test_dataset.targets[i] for i in test_mask[0]])
